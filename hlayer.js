(function(){
    var type = ['msg','alert','loading','iframe','prompt','photo','tips','music', 'render'];
    var utils = {
    ...
    var hlayer = {
        ...
        alert: function(cfg) {
            ...
            if(config.confirmBtn !== false){
                ...
                changeCfg.btnCb.push(config.confirmCb || function () {});
            }
            if(config.cancelBtn) {
                changeCfg.btn.push('取消');
                changeCfg.btnCb.push(config.cancelCb || function () {});
            }
            var setting = utils.mergeJson(changeCfg,config,noChangeCfg);
            return new Cla(setting);
        },
        loading: function(cfg) {
            ...
            var changeCfg = {height:'100px',width:'100px',time:2000,shadow:false,loadingColor:'#2d8cf0'};
            var setting = utils.mergeJson(changeCfg,config,noChangeCfg);
            return new Cla(setting);
        },
        iframe: function(cfg) {
            ...
            var changeCfg = {height:'500px',width:'700px',time:false,shadow:false,closeBtn:true,url:''};
            var setting = utils.mergeJson(changeCfg,config,noChangeCfg);
            return new Cla(setting);
        },
        prompt:function(cfg) {
        ...
        render: function (cfg) {
            var config = cfg || {};
            var noChangeCfg = {type:'render',icon:false,btn:false,text:false};
            var changeCfg = {height:'618px',width:'1080px',time:false,shadow:true,closeBtn:true,animateType:3};
            var setting = utils.mergeJson(changeCfg,config,noChangeCfg);
            return new Cla(setting);
        },
        photo:function(cfg) {
    ...
    Cla.prototype = {
        ...
        layout:function(){
            ...
            if(that.config.type == type[7]){
                ...
                utils.css(audio,{marginLeft: '100px',width: 'calc(100% - 100px)', top: '15px', position: 'relative'});
                that.layerMain.appendChild(audio);
            }
            // 自定义传入内容
            if (that.config.type == type[8]) {
                var render = utils.creEle('div','hlayer-content-render');
                ...
                switch (typeof content) {
                    ...
                    case 'object':
                        var domObj = content;
                        if (typeof domObj.html === 'function') {
                            domObj = domObj[0];
                        }
                        render.appendChild(domObj);
                        break;
                    default:
                        render.innerHTML = content;
                        break;
                }
                that.layerMain.appendChild(render);
            }
        },
        setStyle:function(){
            var that = this;
        ...
        setHeight:function(){
            ...
            if(that.config.width){
                if (typeof that.config.width === 'number') {
                    that.layerCon.style.width = that.config.width + 'px';
                }
            ...
            if(that.config.height) {
                if (typeof that.config.height === 'number') {
                    that.layerCon.style.height = that.config.height + 'px';
                }
                if (typeof that.config.height === 'string') {
